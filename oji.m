...
% Uczenie sieci WTA z regułą Grossberga
for iter = 1:l_iter
    eta = initial_eta * (1 - iter / l_iter);  % malejące tempo uczenia
    total_error = 0;

    for i = 1:size(train_data, 1)
        x = train_data(i, :)';  % wejście jako wektor kolumnowy

        % Oblicz odległości Euklidesowe do wszystkich neuronów
        distances = vecnorm(weights - x', 2, 2);
        [min_dist, winner_idx] = min(distances);  % neuron zwycięzca

        % --- Reguła Grossberga ---
        % Wzór: Δw = η * (x - w)
        weights(winner_idx, :) = weights(winner_idx, :) + eta * (x' - weights(winner_idx, :));

        % Nie normalizujemy wag!
        
        total_error = total_error + min_dist;  % błąd = odległość do zwycięzcy
    end

    % Średni błąd na epokę
...
fprintf('Czułość (Sensitivity): %.2f%%\n', sensitivity * 100);
fprintf('Specyficzność (Specificity): %.2f%%\n', specificity * 100);
